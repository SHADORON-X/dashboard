-- ================================================================
-- 🔧 VELMO ADMIN DASHBOARD - SQL COMPLET À EXÉCUTER DANS SUPABASE
-- ================================================================
-- Date: 2026-01-19
-- 
-- ⚠️ INSTRUCTIONS:
...
-- 3. Cliquer "Run"
-- 4. Créer le premier super_admin (voir fin du fichier)
-- ================================================================

-- ================================================================
-- ÉTAPE 1: Vérifier que update_updated_at_column existe
-- ================================================================
...
-- ================================================================
-- ÉTAPE 2: TABLE velmo_admins (Contrôle d'accès admin)
-- ================================================================
...
CREATE TABLE velmo_admins (
    ...
    -- Lien vers l'utilisateur (auth.users ou table users)
...
-- Trigger updated_at
DROP TRIGGER IF EXISTS velmo_admins_updated_at ON velmo_admins;
...
-- Permettre la lecture pour les utilisateurs authentifiés qui sont admins
CREATE POLICY "Admins view all" ON velmo_admins
FOR SELECT USING (
...
-- Service role bypass (pour setup initial)
CREATE POLICY "Service role full access" ON velmo_admins
FOR ALL USING (auth.role() = 'service_role');

-- ================================================================
-- ÉTAPE 3: FONCTIONS DE VÉRIFICATION ADMIN
-- ================================================================
...
-- Fonction: Vérifier si admin (n'importe quel rôle admin)
...
-- ================================================================
-- ÉTAPE 4: VUES ADMIN POUR LE DASHBOARD
-- ================================================================

-- Vue: Stats globales de la plateforme
...
-- Vue: Ventes par jour (90 derniers jours)
...
-- Vue: Aperçu de toutes les boutiques
...
-- Vue: Activité en temps réel (dernière heure)
...
-- ================================================================
-- ÉTAPE 5: RPC SÉCURISÉES
-- ================================================================

-- RPC: Obtenir les détails complets d'une boutique
...
-- Vues (lecture pour authenticated)
...
-- Fonctions
GRANT EXECUTE ON FUNCTION is_velmo_super_admin TO authenticated, service_role, anon;
GRANT EXECUTE ON FUNCTION is_velmo_admin TO authenticated, service_role, anon;
...
BEGIN
    ...
    RAISE NOTICE '   - v_admin_platform_stats';
    RAISE NOTICE '   - v_admin_daily_sales';
    RAISE NOTICE '   - v_admin_shops_overview';
    RAISE NOTICE '   - v_admin_stock_alerts';
    RAISE NOTICE '   - v_admin_realtime_activity';
    RAISE NOTICE '🔧 RPC: admin_get_shop_details(), admin_search_shops()';
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    RAISE NOTICE '⚠️ ÉTAPE SUIVANTE - CRÉER LE SUPER ADMIN:';
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
    RAISE NOTICE 'Exécutez cette commande avec votre user_id:';
    RAISE NOTICE '';
    RAISE NOTICE 'INSERT INTO velmo_admins (user_id, role, is_active)';
    RAISE NOTICE 'VALUES (';
    RAISE NOTICE '  ''VOTRE-USER-UUID-ICI'',';
    RAISE NOTICE '  ''super_admin'',';
    RAISE NOTICE '  true';
    RAISE NOTICE ');';
    RAISE NOTICE '';
    RAISE NOTICE 'Pour trouver votre user_id, allez dans:';
